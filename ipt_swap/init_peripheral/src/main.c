...
/* CS procedures completed per granted turn before yielding back to the central. Larger =
 * more ranging per (expensive) turn switch, so higher throughput at the cost of coarser
 * per-anchor update granularity. */
#define CS_TURN_BURST 12
...
/* Round-robin turn state: the turn thread waits for a GO grant, enables a bounded burst
 * of CS procedures, then disables and notifies the central DONE. */
static K_SEM_DEFINE(sem_turn_go, 0, 1);
static K_SEM_DEFINE(sem_burst_complete, 0, 1);
static volatile bool turn_active;
static volatile uint32_t turn_proc_count;
...
static void subevent_result_cb(struct bt_conn *conn, struct bt_conn_le_cs_subevent_result *result)
{
	...
	if (result->header.procedure_done_status == BT_CONN_LE_CS_PROCEDURE_COMPLETE) {
		...
		/* Count procedures within the current granted turn; once the burst is done,
		 * signal the turn thread to disable CS and yield back to the central. */
		if (turn_active && ++turn_proc_count >= CS_TURN_BURST) {
			turn_active = false;
			k_sem_give(&sem_burst_complete);
		}
	}
}

/* CS-Turn GATT service. The central writes 1 ("GO") to grant a ranging turn; we run a
 * bounded burst then notify 1 ("DONE") so the central advances to the next link. */
			  ...
			  uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	if (len >= 1 && ((const uint8_t *)buf)[0] == 1) {
		turn_proc_count = 0;
		k_sem_give(&sem_turn_go);
	}
	return len;
}
...
/* Turn thread: on a GO grant, enable a bounded burst of CS procedures, then disable and
 * notify the central DONE. Enabling/disabling CS must run in thread (not callback) context. */
static void turn_thread(void)
{
	while (true) {
		k_sem_take(&sem_turn_go, K_FOREVER);
		...
		if (err) {
			LOG_WRN("Turn: enable procedures failed (err %d)", err);
			turn_active = false;
			continue;
		}

		/* Wait for the burst to complete (counted in subevent_result_cb). Bounded by a
		 * timeout so a stalled burst still yields the turn back. */
		if (k_sem_take(&sem_burst_complete, K_MSEC(1500)) != 0) {
			LOG_WRN("Turn: burst timed out; yielding.");
			turn_active = false;
		}

		struct bt_le_cs_procedure_enable_param dis = {.config_id = CS_CONFIG_ID,
							      .enable = 0};
		(void)bt_le_cs_procedure_enable(conn, &dis);

		/* Notify the central that this link's turn is done. */
		uint8_t done = 1;
		(void)bt_gatt_notify(conn, &cs_turn_svc.attrs[2], &done, sizeof(done));
	}
}
K_THREAD_DEFINE(turn_tid, 2048, turn_thread, NULL, NULL, NULL, 7, 0, 0);
...
static void security_enable_cb(struct bt_conn *conn, uint8_t status)
{
	...
	LOG_INF("CS ready; waiting for turn grants from the central.");
}
