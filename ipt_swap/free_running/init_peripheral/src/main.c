...
/* CS-Turn service: the central writes a "GO" byte to grant this initiator a ranging
 * turn; this initiator runs a bounded burst of CS procedures then notifies "DONE" so
 * the central can advance to the next link (round-robin serialization). */
#define CS_TURN_SVC_UUID                                                                           \
	BT_UUID_128_ENCODE(0xc5710001, 0x7a11, 0x4ee5, 0x9a55, 0x000000000001)
#define CS_TURN_CHR_UUID                                                                           \
	BT_UUID_128_ENCODE(0xc5710002, 0x7a11, 0x4ee5, 0x9a55, 0x000000000001)
static const struct bt_uuid_128 cs_turn_svc_uuid = BT_UUID_INIT_128(CS_TURN_SVC_UUID);
static const struct bt_uuid_128 cs_turn_chr_uuid = BT_UUID_INIT_128(CS_TURN_CHR_UUID);

/* FREE-RUNNING FAIRNESS: CS stays enabled continuously on every link. The central assigns
 * each link a DISTINCT procedure interval (connection-events between CS procedures) via the
 * GATT characteristic so the links de-correlate and the SDC interleaves them instead of
 * starving some. No per-turn enable/disable. */
...
/* Cadence assignment: the central writes this link's procedure interval to the GATT
 * characteristic; the cadence thread then enables CS continuously at that interval. */
static K_SEM_DEFINE(sem_cadence_assigned, 0, 1);
...
static void distance_estimates_update(void)
{
	...
	for (uint8_t i = 0; i < CS_DE_NUM_CHANNELS; i++) {
		...
		/* Count channels that carry a HIGH-quality tone. Must be done BEFORE
		 * cs_de_ifft(), which overwrites iq.values via the scratch_mem union. */
...
static void subevent_result_cb(struct bt_conn *conn, struct bt_conn_le_cs_subevent_result *result)
{
	...
	if (result->header.procedure_done_status == BT_CONN_LE_CS_PROCEDURE_COMPLETE) {
		...
		last_complete_ms = k_uptime_get(); /* feed the starvation watchdog */
	}
}

/* CS-Cadence GATT service. The central writes one byte = this link's CS procedure interval
 * (connection-events between procedures). The cadence thread applies it and enables CS. */
static ssize_t turn_write(struct bt_conn *conn, const struct bt_gatt_attr *attr, const void *buf,
			  uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	if (len >= 1) {
		assigned_proc_interval = ((const uint8_t *)buf)[0];
		k_sem_give(&sem_cadence_assigned);
	}
	return len;
}
...
{
	LOG_INF("CS-Turn notifications %s.", value == BT_GATT_CCC_NOTIFY ? "enabled" : "disabled");
}

BT_GATT_SERVICE_DEFINE(cs_turn_svc, BT_GATT_PRIMARY_SERVICE(&cs_turn_svc_uuid),
		       BT_GATT_CHARACTERISTIC(&cs_turn_chr_uuid.uuid,
					      BT_GATT_CHRC_WRITE | BT_GATT_CHRC_WRITE_WITHOUT_RESP |
						      BT_GATT_CHRC_NOTIFY,
					      BT_GATT_PERM_WRITE, NULL, turn_write, NULL),
		       BT_GATT_CCC(turn_ccc_changed, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE));

/* Cadence thread: wait for the central's procedure-interval assignment, then set procedure
 * parameters at that interval and enable CS CONTINUOUSLY (max_procedure_count=0). Runs in
 * thread (not callback) context as required for HCI commands. Re-applies if reassigned. */
static void cadence_thread(void)
{
	while (true) {
		k_sem_take(&sem_cadence_assigned, K_FOREVER);

		struct bt_conn *conn = connection;
		uint8_t pi = assigned_proc_interval;
		...
		const struct bt_le_cs_set_procedure_parameters_param procedure_params = {
			...
			.min_procedure_interval = pi,
			.max_procedure_interval = pi,
			.max_procedure_count = 0, /* continuous free-running */
		...
		int err = bt_le_cs_set_procedure_parameters(conn, &procedure_params);
		...
		LOG_INF("CS enabled continuously, procedure_interval=%u (err %d)", pi, err);
	}
}
K_THREAD_DEFINE(cadence_tid, 2048, cadence_thread, NULL, NULL, NULL, 7, 0, 0);
...
static void security_enable_cb(struct bt_conn *conn, uint8_t status)
{
	...
	LOG_INF("CS security enabled; awaiting procedure-interval assignment from the central.");
	k_sem_give(&sem_cs_security_enabled);

	/* FREE-RUNNING FAIRNESS: procedure parameters + continuous CS enable are deferred to the
	 * cadence thread, which applies the central-assigned per-link procedure interval. */
}
				...
				struct bt_conn_le_cs_procedure_enable_complete *params)
{
	...
	if (status == BT_HCI_ERR_SUCCESS) {
		if (params->state == 1) {
			cs_running = true;
			last_complete_ms = k_uptime_get(); /* grace period before first complete */
			LOG_INF("CS procedures enabled (interval %u).", params->procedure_interval);
		} else {
			cs_running = false;
			k_sem_give(&sem_proc_disabled);
...
/* Starvation watchdog: a link that lost the scheduling lottery stays CS-enabled but produces
 * no completed procedures. Re-roll its CS phase (disable -> wait disabled -> enable) until it
 * gets a foothold; on the healthy path last_complete_ms keeps advancing so this never fires. */
static void starve_watchdog(void)
{
	while (true) {
		k_sleep(K_SECONDS(2));
		...
		if (k_uptime_get() - last_complete_ms < STARVE_TIMEOUT_MS) {
			continue;
		}

		LOG_WRN("Starved (no CS procedure for >%d ms); re-rolling CS phase.",
			STARVE_TIMEOUT_MS);

		struct bt_le_cs_procedure_enable_param dis = {.config_id = CS_CONFIG_ID, .enable = 0};

		k_sem_reset(&sem_proc_disabled);
		if (bt_le_cs_procedure_enable(conn, &dis) == 0) {
			(void)k_sem_take(&sem_proc_disabled, K_MSEC(1500));
		}

		struct bt_le_cs_procedure_enable_param en = {.config_id = CS_CONFIG_ID, .enable = 1};

		(void)bt_le_cs_procedure_enable(conn, &en);
		last_complete_ms = k_uptime_get(); /* fresh grace period after the re-roll */
	}
}
...
static void cs_config_get(struct bt_le_cs_create_config_params *config_params)
{
	...
	config_params->cs_enhancements_1 = 0; /* SPIKE: IPT off, isolate role issue */
}
...
int main(void)
{
	...
	/* ROLE-SWAP SPIKE: advertise as the BLE peripheral; the CS reflector (central)
	 * connects to us. After connection, the CS-initiator setup below runs unchanged. */
	...
	/* ROLE-SWAP SPIKE (central-driven): the central (reflector) drives ACL security, CS
	 * capability exchange, config creation (with IPT), CS security and procedure enable.
	 * This peripheral initiator only sets its own CS role, then computes IFFT distances
	 * from the subevent results it receives. */
	...
	LOG_INF("Initiator ready (peripheral); central drives CS setup. Computing IFFT...");
